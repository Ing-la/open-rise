...
const PHASE_PROMPTS = {
  ...
  cross: '现在进入对辩环节，你和对方三辩多轮交替发言。发言次数、双方字数等数据详见下方对话记录中的统计。参考战术分析中的字数建议来规划本轮篇幅——一针见血还是展开讲逻辑。\n如果你方字数用完，本轮失去发言权；如果对方字数先用完，你方依然可以继续输出直到自己也耗尽——可以一次打完所有剩余字数，也可以继续保持多轮短促输出。',
  free: '现在是自由辩论环节，双方交替发言，节奏紧凑。每次随机选派一名辩手出场，现在轮到你。\n\n参考战术分析中的字数建议来规划本轮篇幅——一针见血还是展开讲逻辑。双方字数数据详见下方对话记录中的统计。\n如果你方字数用完，本轮失去发言权；如果对方字数先用完，你方依然可以继续输出直到自己也耗尽——可以一次打完所有剩余字数，也可以继续保持多轮短促输出。',
  closing: {
