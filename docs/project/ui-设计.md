...
| 大 OpenRise 标题 | `justify-center` + translateY(-40px) | 3 层 `<h1>` 叠加，`charcoal` 滤镜，略高于垂直中心 |
| chat 模式名称 | absolute left-1/2 + translateX 动态偏移 | AvatarIcon(32px) + text-lg, whitespace-nowrap; 侧边栏打开时右移 128px |
...
- 站立列（左，w-28，absolute 覆盖层）：门 + 站立人物列表（AvatarIcon 48px + 名字），侧边栏打开时隐藏
  - 点击门 → 人物 translateY 缩进门里/从门里下来，错峰延迟
- 输入框全局居中（不受站立列影响），侧边栏打开时随 flex-1 右移
